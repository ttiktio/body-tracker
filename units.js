...
(function(){
  ...
  drawChart=function(canvas,metric,range){
    if(!canvas)return;const ctx=canvas.getContext('2d'),dpr=window.devicePixelRatio||1,cssW=canvas.clientWidth||320,cssH=Number(canvas.dataset.chartHeight)||(canvas.id==='mainChart'?260:220);canvas.dataset.chartHeight=cssH;
    canvas.width=cssW*dpr;canvas.height=cssH*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,cssW,cssH);
    const pts=filterByRange(sorted(),range).map(m=>({date:m.date,value:getMetricValue(m,metric)})).filter(x=>Number.isFinite(x.value));ctx.font='12px -apple-system, sans-serif';ctx.fillStyle='#a1afa5';
    ...
    ctx.strokeStyle='#344236';ctx.lineWidth=1;ctx.beginPath();for(let i=0;i<4;i++){const yy=pad.t+i*(cssH-pad.t-pad.b)/3;ctx.moveTo(pad.l,yy);ctx.lineTo(cssW-pad.r,yy);const val=max-i*(max-min)/3;ctx.fillStyle='#a1afa5';ctx.textAlign='right';ctx.fillText(val.toFixed(1),pad.l-8,yy+4)}ctx.stroke();
    const fill=ctx.createLinearGradient(0,pad.t,0,cssH-pad.b);fill.addColorStop(0,'rgba(196,239,135,.16)');fill.addColorStop(1,'rgba(196,239,135,0)');ctx.fillStyle=fill;ctx.beginPath();ctx.moveTo(x(0),cssH-pad.b);pts.forEach((p,i)=>ctx.lineTo(x(i),y(p.value)));ctx.lineTo(x(pts.length-1),cssH-pad.b);ctx.closePath();ctx.fill();
    ctx.strokeStyle='#c4ef87';ctx.lineWidth=2.5;ctx.beginPath();pts.forEach((p,i)=>i===0?ctx.moveTo(x(i),y(p.value)):ctx.lineTo(x(i),y(p.value)));ctx.stroke();ctx.fillStyle='#c4ef87';pts.forEach((p,i)=>{ctx.beginPath();ctx.arc(x(i),y(p.value),4,0,Math.PI*2);ctx.fill()});
    const locale=lang==='th'?'th-TH':'en-US',first=pts[0],last=pts[pts.length-1];ctx.fillStyle='#a1afa5';ctx.textAlign='left';ctx.fillText(new Date(first.date+'T00:00:00').toLocaleDateString(locale,{day:'numeric',month:'short'}),pad.l,cssH-10);if(pts.length>1){ctx.textAlign='right';ctx.fillText(new Date(last.date+'T00:00:00').toLocaleDateString(locale,{day:'numeric',month:'short'}),cssW-pad.r,cssH-10)}
  };
  ...
  function updateGuide(part){
    selectedGuide=part||'waist';
    $('anatomyGuide').dataset.part=selectedGuide;
    $('anatomyGuide').dataset.sex=settings.sex;
    $('stagePartLabel').textContent=t(selectedGuide);
    $('guideStep').textContent=String(['neck','chest','waist','hip','arm','thigh'].indexOf(selectedGuide)+1).padStart(2,'0');
    document.querySelectorAll('.measure-label').forEach(el=>el.classList.toggle('active-measure',el.querySelector('input')?.id===selectedGuide));
    document.querySelectorAll('.guide-chip').forEach(el=>el.setAttribute('aria-pressed',String(el.dataset.guide===selectedGuide)));
    document.dispatchEvent(new CustomEvent('body-guide-change',{detail:{part:selectedGuide,sex:settings.sex}}));
  ...
  function applyLanguage(){
    ...
    $('langToggle').setAttribute('aria-label',lang==='th'?'Switch to English':'เปลี่ยนเป็นภาษาไทย');
    $('anatomyGuide').setAttribute('aria-label',t('canvasLabel'));
    document.querySelector('.anatomy-controls').setAttribute('aria-label',t('viewControls'));
    $('anatomyImage').alt=t('anatomyAlt');
    $('anatomyZoomIn').setAttribute('aria-label',t('zoomIn'));
    $('anatomyZoomOut').setAttribute('aria-label',t('zoomOut'));
    $('anatomyZoomReset').setAttribute('aria-label',t('resetZoom'));
    document.dispatchEvent(new CustomEvent('body-language-change'));
    renderHome();renderHistory();renderMainChart();updateGuide(selectedGuide);
  }

  window.selectBodyGuide=updateGuide;
  window.bodyTrackerText=t;
  document.querySelectorAll('.guide-chip').forEach(btn=>btn.addEventListener('click',()=>updateGuide(btn.dataset.guide)));
  document.querySelectorAll('[data-show-guide]').forEach(btn=>btn.addEventListener('click',()=>{updateGuide(btn.dataset.showGuide);$('anatomyGuide')?.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'center'})}));
